...
//    public void addChildItemList(SampleGroupBean groupBean,List<SampleChildBean> childBeanList ){
//        if (groupBean.isExpandable()&&isGroupExpanding(groupBean)){
//            int groupPosition = getAdapterPosition(getGroupIndex(groupBean));
//            //notifyItemRangeRemoved(groupPosition + 1, groupBean.getChildCount());
//            groupBean.addChildBean(childBeanList);
////            mExpandGroupSet.add(groupBean);
////           notifyItemRangeInserted(groupPosition + 1, groupBean.getChildCount());
////            notifyItemChanged(groupPosition, EXPAND_PAYLOAD);
